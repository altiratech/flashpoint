...
Current state:
- `main` is pushed to `origin/main`; latest hardening commits are `0bf47bf` (deployed smoke diagnostics), `ce18bac` (deploy verification evidence), and `0e54056` (deploy verification run summary).
...
- Deploy verifies remote D1 schema before Worker deploy and now writes a compact GitHub run summary plus `flashpoint-deploy-verification-*` artifacts with `evidence.json` and `verify-deploy.log` from production API/web verification.
...
- Passed today for deploy-summary hardening: summary render smoke against real/missing evidence, `python3 -m py_compile scripts/summarize-deploy-evidence.py`, production `./scripts/verify-deploy.sh`, `git diff --check`, `npm run lint`, `npm test`, and `npm run build`.
...
- GitHub Deploy run `25576899244` succeeded for `0e54056`; the new run-summary step passed and artifact `6887817026` proved health, bootstrap, profile, episode start, web shell, and bundle API-origin wiring.
