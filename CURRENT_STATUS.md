...
Current state:
- `main` is pushed to `origin/main`; latest hardening commits are `68414fe` (manual production rate-limit smoke), `dbcbddc` (bounded expired-bucket pruning), and `0bf47bf` (deployed smoke diagnostics).
...
- Passed today for smoke-diagnostics hardening: `git diff --check`, `npm run lint`, expected-failure smoke summary write to `/tmp/flashpoint-smoke-fail-test`, `npm test`, and `npm run build`.
...
- GitHub Deploy run `25569909451` succeeded for `0bf47bf`; Deployed Browser Smoke run `25570006430` passed and uploaded diagnostics artifact `6885058858` with screenshots, `github-context.txt`, `browser-smoke.log`, `telemetry-verify.log`, and smoke summaries.
