...
Current state:
- `main` is pushed to `origin/main`; latest hardening commits are `dbcbddc` (bounded expired-bucket pruning), `0bf47bf` (deployed smoke diagnostics), and `ce18bac` (deploy verification evidence).
...
- Deploy verifies remote D1 schema before Worker deploy and now uploads `flashpoint-deploy-verification-*` artifacts with `evidence.json` plus `verify-deploy.log` from production API/web verification.
...
- Passed today for deploy-evidence hardening: `bash -n scripts/verify-deploy.sh`, local production `./scripts/verify-deploy.sh`, expected-failure evidence write, `git diff --check`, `npm run lint`, `npm test`, and `npm run build`.
...
- GitHub Deploy run `25573393332` succeeded for `ce18bac` and uploaded artifact `6886446435` containing `evidence.json` plus `verify-deploy.log`; evidence proved health, bootstrap, profile, episode start, web shell, and bundle API-origin wiring.
