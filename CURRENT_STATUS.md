...
Current state:
- `main` is pushed to `origin/main`; latest hardening commits are `c9a2f6f` (fresh telemetry gate inside deployed smoke), `a419279` (remote D1 schema verifier), and `68414fe` (manual production rate-limit smoke).
- Linear `ALT-38` is implemented through deployed API/web verification: D1-backed `rate_limit_buckets`, route-normalized API rate-limit keys, ordered remote D1 migrations, retry/idempotency tests, bootstrap ETag caching, Pages + Worker smoke coverage, and a production 429 proof.
...
- Passed today for rate-limit smoke hardening: `node --check scripts/verify-rate-limit-remote.mjs`, `git diff --check`, `npm run lint`, `npm test`, `npm run build`, and local `npm run verify:rate-limit:remote`.
